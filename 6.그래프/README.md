...
3. U와 인접한 모든 정점 V에 대해, 만약 V의 거리 값이 (U, V)의 에지 가중치보다 크면 V의 거리 값을 (U, V)의 에지 가중치로 설정한다. 아래 이미지에서 1번 정점과 연결된 2,5번 정점의 거리 값이 각각 2,3으로 변경된 것을 볼 수 있다. 이러한 과정을 정점 U에 안착(settle)했다고 함.
