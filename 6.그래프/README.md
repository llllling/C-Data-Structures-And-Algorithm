...
#### 이분 그래프를 판별하는 알고리즘은 DFS를 조금 변형하여 만들 수 있다.
...
* **크루스칼은 디스조인트-셋 자료구조를 사용해서 사이클 여부 판단해서 MST에 추가하고, 프림은 방문한 정점을 set에 넣어서 방문여부 체크를 해서 MST에 추가하는 점이 다른듯!**
<table>
    <tr>
        <th>크루스칼 알고리즘</th>
        <th>프림 알고리즘</th>
...
* 시간복잡도 O(E log V) --> 개선된 다익스트라 알고리즘 (우선순위 큐 사용)
...
### 다익스트라 알고리즘의 본질적인 문제점
```
음수 에지 가중치를 갖는 그래프에서 제대로 동작을 못함.
왜냐면 그래프를 순회할 때 그리디 방식으로 정점을 선택하므로
```
<div><img src="./dijkstra5.jpg" width="400" height="300"/></div>

* 위 이미지에서 실제로는 B => D => C 경로로 이동해야 가중치가 더 작으므로 더 나은 선택임.
## 벨만-포드 알고리즘
* bellman_ford.cpp
* 음수 가중치가 있는 그래프를 다룰 때 사용할 수 있다.
* 이 방법은 그래프의 모든 에지에 대해 다익스트라의 그리디 선택 방법을 (V - 1)번 반복하여 점진적으로 최단 거리를 찾음.
    * V는 정점의 개수를 의미함.
* **다익스트라와 차이점은 매 반복마다 모든 간선을 확인한다는 것이다. 다익스트라는 방문하지 않은 노드 중에서 최단 거리가 가장 가까운 노드만을 방문한다.**
* 다익스트라 알고리즘보다 높은 점근적 시간 복잡도를 가지지만, 다익스트라 알고리즘이 잘못 해석할 수 있는 그래프에 대해서도 정확한 결과를 제공함.
    * 시간 복잡도가 느리다. O(VE)
* **주어진 그래프가 음수 에지 가중치를 가지고 있지 않다면 그리디 접근 방식을 사용하는 다익스트라 알고리즘이 더욱 효율적임.**
## 벨만-포드 알고리즘과 음수 가중치 사이클
* negative_edge_cycle.cpp : 음수 가중치 사이클 찾기
<div><img src="./bellman1.jpg" width="400" height="300"/></div>

* 빨간색으로 그려진 에지는 음수 가중치 사이클을 구성함.
* **음수 가중치 사이클** : 에지 가중치 합이 음수가 되는 사이클을 의미함.
* 그래프에 음수 가중치 사이클이 있을 경우, 해당 사이클이 반복적으로 계산되어 최종 결과가 왜곡될 수 있음.
## 존슨 알고리즘
* 다익스트라 알고리즘의 효율성을 활용함과 동시에 음수 가중치를 갖는 그래프에 대해서도 올바른 결과를 생성할 수 있다.
* 다익스트라 알고리즘의 한계를 극복하기 위해 전체 에지 가중치를 음수가 아닌 형태로 변환함.
* 첫 번째 단계는 그래프에 새로운 더미(dummy)정점을 추가하는 것. 그리고 이 더미 정점과 나머지 모든 정점 사이에 가중치가 0인 에지를 연결함. 이후 벨만-포드 알고리즘을 이용하여 더미 정점과 나머지 정점들 사이의 최단 경로를 찾고, 나중에 사용하기 위해 각 정점까지의 최단 거리를 기록함.